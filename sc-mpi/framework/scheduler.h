...
#else // Not using SSE.
  msg_buffer_scalar<MessageValue>* msg_buf;
#endif
...
scheduler(graph<VertexValue, EdgeValue> *g, int my_rank) {
...
#else  // SCALAR
    this->num_buffers = g->num_vertices;
#endif
  }
  ...
  #ifdef MSG_INT
    msg_buf = new msg_buffer_scalar<MessageValue>[num_buffers];
  #elif defined MSG_FLOAT 
    msg_buf = new msg_buffer_scalar<MessageValue>[num_buffers];
  #elif defined MSG_DOUBLE
    msg_buf = new msg_buffer_scalar<MessageValue>[num_buffers];
...
void scheduler<VertexValue, EdgeValue, MessageValue>::reorder() {
  ...
  // If scalar, we don't need to sort.
#if defined (SCALAR)
  int max_deg = 0;
  for (int i = 0; i < g->num_vertices; ++i) {
    max_deg = max(g->in_degree[i].in_degree, max_deg);
  }
  cout << "Max degree: " << max_deg << endl;
  for (int i = 0; i < g->num_vertices; ++i) {
    msg_buf[i].set_length(g->in_degree[i].in_degree);
  } 
  return;
#endif
