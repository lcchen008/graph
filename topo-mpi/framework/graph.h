...
class graph {
    ...
    vector<int> local_to_global_map;
		...
		// Indicate a vertex is active or not.
		atomic<bool>* status;
		...
		void deactive_all_vertices();

    void set_status(size_t global_vertex_id, bool status);
...
graph<VertexValue, EdgeValue>::graph(size_t global_num_vertices,
  ...
  for(int i = 0; i < num_vertices; i++) {
	...
	this->status = new atomic<bool>[num_vertices];
  edge_value = 0;
  vertex_value = 0;
  global_to_local_map = 0;
  deactive_all_vertices();
}
...
template <class VertexValue, class EdgeValue>
void graph<VertexValue, EdgeValue>::deactive_all_vertices() {
	// memset(&status[0], 0, sizeof(bool) * num_vertices);	
  for (int i = 0; i < num_vertices; ++i) {
    status[i] = 0;
  }
}
...
template <class VertexValue, class EdgeValue>
void graph<VertexValue, EdgeValue>::set_status(size_t global_vertex_id,
                                               bool st) {
