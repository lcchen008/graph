// This tool transforms the livejournal graph input to 0-based format.
...
// Raw livejournal input is 0 based. 
void TransGraph(int num_vertices,
  ...
  while (getline(input, line)) {
    ...
    //the first is the source
		ss >> first;
		vid = atoi(first.c_str());
		ss >> second;
		edge = atoi(second.c_str());
		if(vid != prev_vid) {
...
int main() {
  TransGraph(4847571,
             "../code/input/livejournal/soc-LiveJournal1.txt",
             "../code/input/livejournal/soc-livejournal-undirected.txt");
}
