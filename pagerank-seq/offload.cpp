...
void graph::page_rank()
{
  ...
  float* sum = new float[num_vertices];
  for (int i = 0; i < num_vertices; ++i) {
    sum[i] = 0;
  }
	double before_msg = rtclock();

  // #pragma offload target (mic) in(locks:length(num_vertices)) out(sum:length(num_vertices))
  // #pragma omp parallel for schedule(dynamic,100) num_threads(nthreads)

  #pragma offload target (mic)
	_Cilk_for(int i = 0; i < num_vertices; i++)
	{
